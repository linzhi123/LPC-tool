...
//	LPC_SYSCON->SYSAHBCLKCTRL |= (1 << 6) | (1 << 16);   
	...
	//LPC_IOCON->PIO1_15 |= 0x3; 

	/* SSP SSEL */
	LPC_IOCON->PIO1_19 &= ~(0x1F); //配置PIO1_19为ssel1
	LPC_IOCON->PIO1_19 |= PINSEL_PIO_FUNC(2) | PINSEL_PIO_MODE(2); 
	...
	LPC_SSP1->CR1 = (LPC_SSP1->CR1 & ~(1 << 2)) | (0<<2);
	
	LPC_SSP1->CR0 = (LPC_SSP1->CR0 & ~0xFF) | (0x7<<0) | (0<<4) | (0<<6) | (1<<7);
	
	Chip_SSP_Set_BitRate(100000);
...
//    tmp = ((SSP_CPHA_USED) | (SSP_CPOL_USED) \
            | (SSP_FRAME_SPI) | (SSP_DATA_BIT_USED) | (0<<8)) \
            & SSP_CR0_BITMASK;
...
//    LPC_SSP1->CR0 = tmp;
    ...
    // Write back to CR1 // SSP Enable
    LPC_SSP1->CR1 = (0<<2) | (1<<1);
		
    // Set clock rate for SSP peripheral
//    LPC_SSP1->CR0 &= (~ SSP_CR0_SCR (0xFF)) & SSP_CR0_BITMASK;
//    LPC_SSP1->CR0 |= (SSP_CR0_SCR(0x05)) & SSP_CR0_BITMASK;
//    LPC_SSP1->CPSR = 0x02 & 0xfe; //SSP_CPSR_BITMASK;		
