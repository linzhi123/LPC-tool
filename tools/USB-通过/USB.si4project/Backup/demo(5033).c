...
void VCOM_Serial2Usb(void) {
	...
	static unsigned char serBuf [256];
	...
	if (numAvailByte > 0) {
		if (CDC_DepInEmpty) {
			numBytesRead = ser_Read ((char*)pBuff, &numAvailByte);
			_PRINT_(pBuff);
			_PRINT_H(numBytesRead);
			CDC_DepInEmpty = 0;	
			while(numBytesRead)
			{
				if (numBytesRead < 64 )
				{
					USB_WriteEP (CDC_DEP_IN, pBuff, numBytesRead);
					break;
				}
				else
				{
					USB_WriteEP (CDC_DEP_IN, pBuff, 64);
					numBytesRead-=64;
					pBuff+=64;
				}
			}
