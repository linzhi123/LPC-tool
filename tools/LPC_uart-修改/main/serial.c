...
void UARTSendByte(unsigned char ucbyte)
{
	LPC_USART->THR = ucbyte;
...
{
	register unsigned char*pstr=str ;
	while(*pstr != 0)
	{
		UARTSendByte(*pstr++);
	}
...
{
  uint8_t BufferPtr;
  while((LPC_USART->LSR & 0x01) == 0);   
  BufferPtr = LPC_USART->RBR;
  return  BufferPtr;
}
