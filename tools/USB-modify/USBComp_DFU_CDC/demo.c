...
void VCOM_Serial2Usb(void)
{
	...
	ser_AvailChar ((int*)&numAvailByte);
	if (numAvailByte > 0)
	{
		if (CDC_DepInEmpty) 
		{					
			numBytesRead = ser_Read ((char*)serBuf, (int*)&numAvailByte);
			//_PRINT_(serBuf);
			CDC_DepInEmpty = 0;	
			while(numBytesRead)
			{
...
void Enable_clk(uint32_t n)
{
	LPC_SYSCON->SYSAHBCLKCTRL |= n;
}
...
int main (void) 
{
	...
	Enable_clk((1 << 6) | (1 << 16));    //打开GPIO和IOCON时钟
